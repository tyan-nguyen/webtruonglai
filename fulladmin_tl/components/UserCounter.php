...
class UserCounter extends Component
{
	...
	public function refresh($force = false)
	{
		...
		if ($this->isNewDay()) {
			...
			//add yesterday table
			$anYesterday = date("Y-m-d", strtotime( '-1 days' ) );
			$anMod = PcounterByDay::find()->where(['day'=>$anYesterday])->one();
			if( $anMod != null){
				$anMod->delete();
			}
			$anCountByDay = new PcounterByDay();
			$anCountByDay->day = date("Y-m-d", strtotime( '-1 days' ) );
			$anCountByDay->user = $this->yesterday;
			$anCountByDay->save();
		}
